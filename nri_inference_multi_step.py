...
def main(args):
    ...
    te_mse = [[] for _ in range(args.pred_steps)];
    te_r2 = [[] for _ in range(args.pred_steps)];
    ...
    time_ellapsed_std = np.average((time_ellapsed - time_ellapsed_mean) ** 2,
                                   weights=weights) if args.train_online else float('nan')
