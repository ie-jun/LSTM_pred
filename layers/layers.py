...
class DilatedInceptionLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_time_series, **kwargs):
        super().__init__()
        # lag길이를 갖는  shape 유지하기 위해서 통과후 size가 변화 없도록 필터 보유
        ...
        # self.branch2x1 = nn.Conv2d(in_channels,out_channels, kernel_size=(2,1), dilation=(dilation_factor,1), groups=in_channels)
        # self.branch3x1 = nn.Conv2d(in_channels,out_channels, kernel_size=(3,1), dilation=(dilation_factor,1), groups=in_channels)
        # self.branch5x1 = nn.Conv2d(in_channels,out_channels, kernel_size=(6,1), dilation=(dilation_factor,1), groups=in_channels)
        # self.branch7x1 = nn.Conv2d(in_channels,out_channels, kernel_size=(7,1), dilation=(dilation_factor,1), groups=in_channels)
...
class GraphConvolutionModule(nn.Module):
    ...
    def __init__(self, in_features, out_features, k, **kwargs):
        ...
        for i in range(k + 1):
            setattr(self, f'info_select{i}',
                    nn.Conv2d(out_features, out_features, 1, 1, 0, 1, groups=out_features, bias=False, **kwargs))

        self.in_features, self.out_features, self.k \
            = in_features, out_features, k

    def norm_adj(self,A):
        if len(A.shape)==2 or len(A.shape) == 3:
            with torch.no_grad():
                if len(A.shape) == 3:
                    eyes = torch.stack([torch.eye(A.shape[1]) for _ in range(A.shape[0])]).to(device = device)
                    D_tilde_inv = torch.diag_embed(1/1.+torch.sum(A,dim=1))
    ...
    def forward(self, x, A, beta=0.5):
        ...
        # x = self.conv_inter(x)
        hiddens = [x]
        #gcl은 informationPropagation 과정임.
