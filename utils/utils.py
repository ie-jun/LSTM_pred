...
def multistep_mtgnn_write_csv(args, path_name, file_name, data, columns= None):
    log_path= os.path.join(args.model_path, path_name)
    os.makedirs(log_path, exist_ok= True)
    log_file= os.path.join(log_path, file_name)
    with open(log_file, 'w', newline= '') as f:
        wr = csv.writer(f)
        n = len(data) # f: n x pred_steps x ts
        pred_steps =data.shape[1]
        ...
        for i in range(n):
            for j in range(pred_steps):
                wr.writerow(data[i, j, :])


def lstm_write_csv(args, path_name, file_name, data,colname):
    log_path= os.path.join(args.model_path, path_name)
    os.makedirs(log_path, exist_ok= True)
    log_file= os.path.join(log_path, file_name)
    with open(log_file, 'w', newline= '') as f:
        wr = csv.writer(f)
        n = len(data)  #data가 예측결과라 (데이터 개수 , 예측 col자체)라 len(data)는 데이터 개수
        # wr.writerow(list(logs.keys()))
