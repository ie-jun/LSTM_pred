...
def train(args,
    ...
    for epoch in range(args.epoch):
        ...
        print(
            f'Epoch [{epoch + 1}/{args.epoch}]: training loss= {tr_loss:.6f}, training mse loss= {tr_mse_loss:.6f}, training bce loss= {tr_bce_loss:.6f}')
        empty = ' ' * len(f'Epoch [{epoch + 1}/{args.epoch}]')
        print(
            f'{empty}: validation loss= {valid_loss:.6f}, validation mse loss= {valid_mse_loss:.6f}, validation bce loss= {valid_bce_loss:.6f}')
...
def multistep_mtgnn_train(args,
          model,
          train_loader, valid_loader,
          optimizer, criterion, early_stopping,
          device):
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, x in enumerate(valid_loader):
            ...
            with torch.no_grad():
                out = model(x, args.beta)
                #multi step 예측이여도 바로 criterion()으로 사용 가능
                mse_loss = criterion(out['outs_label'], x['label'])
                if out['outs_mask'] is not None:
                    bce_loss = criterion_mask(out['outs_mask'], x['label_mask'])
                    loss = mse_loss + bce_loss
...
def lstm_train(args,
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, x in enumerate(train_loader):
            ...
            with torch.set_grad_enabled(True):
                out = model.forward(x)
                # out과 x['label'] shape 맞추기.
                mse_loss = criterion(out, x['label'].view([-1,1]))
                loss = mse_loss
        ...
        print(
            f'Epoch [{epoch + 1}/{args.epoch}]: training loss= {tr_loss:.6f}')
        empty = ' ' * len(f'Epoch [{epoch + 1}/{args.epoch}]')
        print(
            f'{empty}: validation loss= {valid_loss:.6f}')
        early_stopping(valid_loss, model, epoch, optimizer)

        if early_stopping.early_stop:
            break

    #모든 epoch만큼 학습이 전부 끝난 후에 saving logs
    print("Training done! Saving logs...")
    log_path = os.path.join(args.model_path, 'training_logs')
    os.makedirs(log_path, exist_ok=True)
    enb_id = args.enb
    colname = args.col
    log_file = os.path.join(log_path, f'training_logs_{enb_id}_{colname}.csv')
    with open(log_file, 'w', newline='') as f:
        wr = csv.writer(f)
        n = len(logs['tr_loss'])    #n은 epoch 개수?
        rows = np.array(list(logs.values())).T
        wr.writerow(list(logs.keys()))
        for i in range(1, n):
            wr.writerow(rows[i, :])
...
def multistep_mtgnn_test_regr(args,
              model,
              test_loader,
              criterion,
              device
    ...
    for batch_idx, x in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # store predictions
            preds.append(out['preds'].detach().cpu())  # bs, c, 예측시점, n
            labels.append(x['label'].detach().cpu())  # bs, c, 예측시점, n
        ...
        te_mae += mean_absolute_error(out['preds'].detach().cpu().numpy().flatten(),
                                      x['label'].detach().cpu().numpy().flatten())
        te_mse += mean_squared_error(out['preds'].detach().cpu().numpy().flatten(),
                                     x['label'].detach().cpu().numpy().flatten())
    ...
    if args.num_folds == 1:
        ...
        preds = torch.concat(preds, dim=0)  # num_obs, num_cells, 예측 개수, num_time_series
        preds =torch.permute(torch.squeeze(preds),(1,0,2,3)) # num_cells, num_obs,예측 개수, num_time_series
        preds = preds.numpy()
        if args.cache is not None:
            # preds = inv_min_max_scaler(preds, args.cache, args.columns)
            preds = inv_min_max_scaler_ver2(preds, args.cache, args.columns)

        labels = torch.concat(labels, dim=0)  # num_obs, num_cells, 예측 개수, num_time_series
        labels = torch.permute(torch.squeeze(labels), (1, 0, 2,3))  # num_cells, num_obs,예측 개수, num_time_series
        labels = labels.numpy()
        if args.cache is not None:
        ...
        for i in tqdm(range(num_cells), total=num_cells):
            ...
            #preds[i,...]은 num_obs,예측 개수, num_time_series의 shape
            #각 관측당 pred_steps만큼의 row를 가지게끔 구현
            multistep_mtgnn_write_csv(args, 'test/predictions', f'predictions_{enb_id}.csv', preds[i, ...], args.columns)
            multistep_mtgnn_write_csv(args, 'test/labels', f'labels_{enb_id}.csv', labels[i, ...], args.columns)

            #csv로 값만 저장하고 시각화는 주피터에서 하기로 함
...
def lstm_test_regr(args,
    ...
    for batch_idx, x in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # store predictions
            preds.append(out.detach().cpu())  # bs, 1
            labels.append(x['label'].view([-1,1]).detach().cpu())  # bs, 1


        te_tot_loss += loss.detach().cpu().numpy()

        te_r2 += r2_score(out.detach().cpu().numpy().flatten(), x['label'].view([-1,1]).detach().cpu().numpy().flatten())
        te_mae += mean_absolute_error(out.detach().cpu().numpy().flatten(),
                                      x['label'].view([-1,1]).detach().cpu().numpy().flatten())
        te_mse += mean_squared_error(out.detach().cpu().numpy().flatten(),
                                     x['label'].view([-1,1]).detach().cpu().numpy().flatten())
    ...
    print(f"mse: {te_mse:.2f}")
    print()
    #.view([-1,1])했는데 저장된거 보면 label이 torch.Size[batch_size])냐 왜..

    if args.num_folds == 1:
        # concatenate predictions
        print('saving the predictions...')
        preds = torch.concat(preds, dim=0)  #batch들이 concat돼서 (num_obs, 1)
        preds = preds.numpy()
        if args.cache is not None:
            # preds = inv_min_max_scaler(preds, args.cache, args.columns)
            #여기가 lstm에서 문제가 된다면 lstm버전의 min_max_scaler만들어야함
            preds = inv_min_max_scaler_ver2(preds, args.cache, args.columns)
        ...
        enb_id = args.enb
        colname = args.col
        lstm_write_csv(args, 'test/predictions', f'predictions_{enb_id}_{colname}.csv', preds,colname)
        lstm_write_csv(args, 'test/labels', f'labels_{enb_id}_{colname}.csv', labels,colname)
